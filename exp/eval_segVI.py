...
def get_args():
    ...
    parser.add_argument('-zr', '--zw-rm', type=str, default=100,
                        help='zwatershed remove small segments')
    parser.add_argument('-ap', '--ag-param', default='2,3,1,0.15,0',
                        help='env for agglomeration')
...
if __name__ == "__main__":
    ...
    print '\t 2.3 remove small seg'
    if not os.path.exists(zw_segRm):
    ...
    ag_suf_train = ag_pref+'_st'+ag_str+'_itr'+ag_itr+'_'+args.zw_thres+'_rm'+args.zw_rm
    ag_suf_test = ag_suf_train+'_'+ag_alg+'_'+ag_thr+'_'+ag_reg+'_'+args.ag_test_pref
    ...
    if args.ag_do_train==1 and not os.path.exists(ag_train):
        c3 = args.eval+args.ag_train+' -watershed '+zw_segRm+' stack -prediction '+predReorder+' stack -classifier '+ag_train+' -groundtruth '+args.seg_gt+' stack -strategy '+ag_str+' -iteration '+ag_itr+' -nomito\n'   
        runBash(c0+c1+c2+c3)
    ...
    if os.path.exists(ag_train) and args.ag_test_pref!='' :
        ...
        if not os.path.exists(ag_test):
            ...
            c3 = args.eval+args.ag_test+' -watershed '+zw_segRm+' stack -prediction '+predReorder+' stack -classifier '+ag_train+' -output '+ag_test+'h stack -algorithm '+ag_alg+' -threshold '+ag_thr+' -min_region_sz '+ag_reg+' -nomito\n'
            runBash(c0+c1+c2+c3)
