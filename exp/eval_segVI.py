...
def get_args():
    ...
    parser.add_argument('-e', '--eval', default='/n/coxfs01/donglai/malis_trans/vi_test/',
                        help='library folder')
    parser.add_argument('-dz', '--do-z', default='zwatershed_script/generate_watershed_affinity_dw.py',
    ...
    parser.add_argument('-dma', '--mean-aff', default='zwatershed_script/mean_aff.py',
    ...
    parser.add_argument('-za', '--zw-aff-thres', type=str, default='0.01_0.8_0.2',
                        help='affinity thresholding for zwatershed')
    ...
    parser.add_argument('-dmE', '--do-maEval', type=int, default=0,
                        help='if evaluate mean affinityzwatershed')
...
if __name__ == "__main__":
    ...
    zw_thres = [int(x) for x in args.zw_thres.split('_')]
    def zw_seg(thres=-1):
        out = args.pred_pref + '-zwater_'+args.zw_aff_thres+'_'+args.zw_dust
        if thres>0:
            out+='_'+str(thres)+'.h5'
        return out

    print '\t 2.1 segmentation'
    if not os.path.exists(zw_seg(zw_thres[-1])):
        call([args.zw_env+'python', args.eval+args.do_z, predUp, args.pred_name, zw_seg(), args.zw_thres, args.zw_aff_thres, args.zw_dust])
