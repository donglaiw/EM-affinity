...
# CUDA_VISIBLE_DEVICES=1,2,3,4,5,6,7,8 python E_train.py 1 16 18125 3625 s0921/16_8_1e-3 im_uint8 seg-groundtruth2-malis 8 0.001 16 iter_16_625.pth
...
# training option
parser.add_argument('--m','--model-opt', type=int, choices=model_names, default=0,
                    help='model type')
parser.add_argument('--l','--loss-opt', type=int, choices=loss_names, default=2,
                    help='loss type')
parser.add_argument('--d','--data-opt', type=int, choices=loss_names, default=2,
                    help='loss type')
parser.add_argument('--lr', type=float, default=0.0001,
                    help='learning rate')
parser.add_argument('--beta', default='0.9,0.99',
                    help='beta for adam')
...
def main():
    ...
    if not os.path.isdir(sn):
        ...
        train_data =  np.array(h5py.File(args.input+args.data_name+'.h5','r')['main'],dtype=np.float32)[None,:]/(2.**8)
        train_nhood = malis_core.mknhood3d()
        if os.path.exists(args.input+args.label_name+'_aff.h5'):
            train_label = np.array(h5py.File(args.input+args.label_name+'_aff.h5','r')['main'])
        else: # pre-compute for faster i/o
            train_seg = np.array(h5py.File(args.input+args.label_name+'.h5','r')['main'])[train_offset[0]-2:-train_offset[0]+2,train_offset[1]-2:-train_offset[1]+2,train_offset[2]-2:-train_offset[2]+2]
            train_label = malis_core.seg_to_affgraph(train_seg,train_nhood)[:,1:-1,1:-1,1:-1]
            from T_util import writeh5
            writeh5(args.input+args.label_name+'_aff.h5', 'main', train_label)
        # add sampler
        nhood = None if args.loss_opt in [0,1] else train_nhood

        color_scale = [(0.8,1.2), (0.9,1.1)][args.data_opt]
        color_shift = [(0.2,1.2), (-0.1,0.1)][args.data_opt]
        train_dataset = VolumeDatasetTrain(train_data, train_label, nhood, args.batch_size, data_size=train_data.shape[1:], 
                                           reflect=(1,1,1),swapxy=True,color_scale=color_scale,color_shift=color_shift,clip=(0.05,0.95),
        ...
        if args.model_opt in [0,1]: # weighted L2 training
            loss_fn = torch.nn.MSELoss()
            loss_suf = '_L2_'+str(lr)
        elif args.model_opt == 2: # malis training
            import malisLoss
        ...
        for iter_id, data in enumerate(train_loader):
            ...
            if (iter_id+1) % args.iter_save == 0: 
                ...
                save_checkpoint(model, optimizer, iter_id, sn+('iter_%d_%d_%s.pth' % (args.batch_size,pre_epoch+iter_id+1,str(lr))))
            iter_id+=1
